class WebMenu {
  constructor() {
    ...
    this._webMenuListContainer = document.querySelector(
  ...
  _navigateWithArrows = (key, len) => {
    ...
    const getIndexByWindowWidth = () => {
      ...
      const containerWindow =
        (window.innerWidth - menuItemWidth / 2 - scrollBarWidth - vw(24)) /
        menuItemWidth;
      // Get rounded result
    ...
    const changeItemFocus = (condition, overFlowIndex) => {
      const next = this._webMenuList.getElementsByTagName("li")[
        this._webListIndex
      ];
      if (typeof next !== "undefined" && condition) {
        ...
        this._webItemFocus = this._webMenuList.getElementsByTagName("li")[
          parseInt(overFlowIndex, 10)
        ];
      }
    };
